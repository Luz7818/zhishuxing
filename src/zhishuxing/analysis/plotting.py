"""绘图基础设施：中文字体、滑窗平均、Agg 后端与枢纽快照渲染。

全仓库唯一一份字体配置与 moving_average 实现（历史上分散在 6+ 个脚本中）。
"""
...
matplotlib.use("Agg")  # 服务端/CLI 全部无头渲染；如需交互式窗口请在本模块导入前自行切换
...
def moving_average(values: Sequence[float], window: int) -> np.ndarray:
    ...
    pad = np.repeat(smoothed[0], window - 1)
