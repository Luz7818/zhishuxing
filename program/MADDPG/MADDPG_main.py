...
class Runner:
    def __init__(self, args, env_name, number, seed):
        ...
        self.env = Env(
            env_name,
            discrete=False,
            behavior_name=self.args.behavior_name,
            file_name=self.args.mlagents_file,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("Hyperparameters Setting for MADDPG and MATD3 in Unity ML-Agents environment")
    parser.add_argument("--mlagents_file", type=str, default=None, help="Unity可执行文件路径；为None时连接Unity Editor")
    parser.add_argument("--behavior_name", type=str, default=None, help="Unity Behavior 名称；为空时自动取第一个")
    parser.add_argument("--base_port", type=int, default=5005, help="Unity通信端口")
    parser.add_argument("--no_graphics", type=bool, default=True, help="是否无图形模式运行")
    ...
    env_names = ["integrated_hub_transfer"]
    env_index = 0
